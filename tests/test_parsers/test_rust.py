...
def test_rust_parser_discards_partial_state_when_one_file_fails(tmp_path, monkeypatch):
    ...
    def fail_for_poisoned_file(container, exclude_tests):
        if b"Poisoned" in container.text:
            raise RuntimeError("synthetic extraction failure")
        return original_extract_imports(container, exclude_tests)
...
def test_rust_parser_skips_compound_cfg_test_predicates(tmp_path):
    """all(test, ...) / any(test, ...) gate test-only code just like cfg(test)."""
    source = tmp_path / "lib.rs"
    source.write_text(
        "pub struct Production;\n"
    ...
    assert "Production" in graph.entities
    assert all("Fixture" not in fqn for fqn in graph.entities)
    assert "all_tests.all_helper" not in graph.entities
    assert "nested_test_helper" not in graph.entities


def test_rust_parser_keeps_cfg_not_test_items(tmp_path):
    """#[cfg(not(test))] marks production-only code and must survive."""
    source = tmp_path / "lib.rs"
    source.write_text(
        "#[cfg(not(test))]\n"
    ...
    assert "ProductionOnly" in graph.entities
    assert "production_only_helper" in graph.entities


def test_rust_parser_skips_files_gated_by_inner_cfg_test(tmp_path):
    """A file whose top is #![cfg(test)] contributes nothing to the graph."""
    source = tmp_path / "lib.rs"
    source.write_text(
        "#![cfg(test)]\n"
        "pub struct Fixture;\n"
    ...
    assert RustParser().parse([source], tmp_path).entities == {}

    included = RustParser(exclude_tests=False).parse([source], tmp_path)
    assert "Fixture" in included.entities


def test_rust_parser_skips_module_bodies_gated_by_inner_cfg_test(tmp_path):
    source = tmp_path / "lib.rs"
    source.write_text(
        "pub struct Production;\n"
        "mod harness {\n"
    ...
    assert "Production" in graph.entities
    assert "harness.Fixture" not in graph.entities
    assert "harness.helper" not in graph.entities


def test_rust_parser_skips_files_behind_out_of_line_cfg_test_modules(tmp_path):
    """#[cfg(test)] mod helpers; must also exclude the helpers.rs file itself."""
    (tmp_path / "Cargo.toml").write_text('[package]\nname = "probe"\nversion = "0.1.0"\n')
    source = tmp_path / "src"
    source.mkdir()
    ...
    (source / "deep").mkdir()
    (source / "deep" / "mod.rs").write_text("pub struct DeepFixture;\n")
    (source / "real.rs").write_text("pub struct RealThing;\n")

    graph = parse(str(tmp_path), language="rust", use_cache=False)

    assert "Production" in graph.entities
    assert "real.RealThing" in graph.entities
    assert all("Fixture" not in fqn for fqn in graph.entities)

    included = parse(str(tmp_path), language="rust", use_cache=False, exclude_tests=False)
    assert "helpers.HelperFixture" in included.entities
    assert "deep.DeepFixture" in included.entities


def test_rust_parser_skips_nested_out_of_line_cfg_test_modules(tmp_path):
    """The excluded path is relative to the declaring module, not the crate root."""
    (tmp_path / "Cargo.toml").write_text('[package]\nname = "probe"\nversion = "0.1.0"\n')
    source = tmp_path / "src"
    (source / "engine").mkdir(parents=True)
    (source / "lib.rs").write_text("pub mod engine;\n")
    (source / "engine" / "mod.rs").write_text(
        "pub struct Engine;\n#[cfg(test)]\nmod fixtures;\n"
    )
    ...
    assert "engine.Engine" in graph.entities
    assert all("Fixture" not in fqn for fqn in graph.entities)


def test_rust_parser_drops_cfg_test_use_declarations(tmp_path):
    """Dev-dependency imports must not attach to production entities."""
    source = tmp_path / "lib.rs"
    source.write_text(
        ...
        "pub struct Prod { map: HashMap<String, String> }\n"
    )

    graph = RustParser().parse([source], tmp_path)

    assert graph.entities["Prod"].imports == ["std::collections::HashMap"]

    included = RustParser(exclude_tests=False).parse([source], tmp_path)
    assert "mockall::predicate::Eq" in included.entities["Prod"].imports


def test_rust_parser_leaks_no_test_entities_across_cfg_test_shapes(tmp_path):
    """End-to-end probe: every recognized #[cfg(test)] shape in one crate."""
    (tmp_path / "Cargo.toml").write_text('[package]\nname = "probe"\nversion = "0.1.0"\n')
    source = tmp_path / "src"
    source.mkdir()
    (source / "lib.rs").write_text(
    ...
    (source / "outofline.rs").write_text(
        "pub struct OutOfLineFixture;\npub fn out_of_line_helper() {}\n"
    )
    (source / "inner_test_file.rs").write_text(
        "#![cfg(test)]\npub struct InnerFixture;\npub fn inner_helper() {}\n"
    )

    graph = parse(str(tmp_path), language="rust", use_cache=False)

    assert set(graph.entities) == {"Prod"}
    assert graph.entities["Prod"].imports == ["std::collections::HashMap"]


def test_rust_parser_lists_each_package_entity_once(tmp_path):
    """The package index is de-duplicated (via a set, not a linear scan)."""
    source = tmp_path / "lib.rs"
    source.write_text(
        "pub struct Repo;\n"
    ...
    for package, fqns in graph.packages.items():
        assert len(fqns) == len(set(fqns)), package


def test_rust_workspace_fixture_ingests_every_member_crate():
    """A Cargo [workspace] root must not be narrowed to its own src/ directory."""
    workspace = Path(__file__).resolve().parents[1] / "fixtures" / "rust_workspace"

    repo = ingest(str(workspace), language="rust")

    assert repo.path == workspace
    assert sorted(str(path.relative_to(workspace)) for path in repo.source_files) == [
    ...
    assert "workspace_root.RootService" in graph.entities
    assert "alpha.AlphaGreeter" in graph.entities
    assert "beta.BetaClient" in graph.entities
