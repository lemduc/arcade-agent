...
it plans a change, edits across files, runs the tests, and iterates. What was
built on the premise that a person stands at every step --- the process, the
review points, the tools, and the guarantees each was trusted to provide --- is
now due for re-examination.
...
architecture. Conformance checking is a mature idea~\cite{murphy1995reflexion,archunit},
but it runs in continuous integration, after the code is written, addressed to a
human reviewer. For an agent that is both too late and too blunt: the code
already exists, and a failed check that carries no remedy produces thrashing
rather than repair. Meanwhile the structural decay such a check is meant to
catch is measurable and consequential, and predicts where defects and costly
change will land~\cite{le2018decay,le2021decay-predictor}.

We present \arcade{}, a tool that moves architecture recovery, smell detection,
decay metrics, and conformance checking inside the agent's loop. The analyses it
performs are established; our contribution is making them something an agent can
afford to call, and can call \emph{before} the code exists rather than after.

\noindent\textbf{Contributions.}
\begin{itemize}
  \item An installable, framework-agnostic architecture analysis library
        (\tool{pip install arcade-agent}) with seven language parsers and five
        recovery algorithms.
  \item An MCP server exposing 19 tools with per-call token budgeting and a
        session-handle protocol, so an agent consumes summaries rather than
        dumps: \arcade{}'s own architecture costs 63{,}586 tokens in full and
        911 through \tool{summarize}.
  \item Four task-shaped context tools that answer the question an agent
        actually has --- what should I read, what does this change break, what is
        public --- instead of returning a graph.
  \item An executable architecture contract and proactive guardrail tools, with
        evidence that they eliminate a violation class an unaided agent
        introduces in roughly one greenfield run in five.
\end{itemize}
